...
namespace SimpleWebPositionApp.Controllers {
    public class Product68Controller : Controller {
        ...
        public async Task<IActionResult> Search(SearchBar bar) {
            ...
            if (top_code == null) {
                return RedirectToAction("error", "product68", new { errorType = "Δεν δόθηκε κωδικός προς αναζήτηση." });
            }
            ...
            if (code == null)
                return RedirectToAction("error", "product68", new { errorType = "Δεν βρέθηκε εγγραφή." });
            var productFile = await _context.Products68.SingleOrDefaultAsync(m => m.TopCode == code.TopCode);

            return productFile == null ? RedirectToAction("error", "product68", new { errorType = "Δεν Βρέθηκε εγγραφή." }) : View(productFile);
        }
